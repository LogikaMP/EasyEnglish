function load_word(){
    ...
    let index = 0
    function load_task(){
        document.querySelector(".word.current").innerHTML = task[index][ln1]
    }
    load_task()
    document.querySelector(".test.button.word").addEventListener("click", function(){
        if(ln1 == "en"){
            ln1 = "ua"
            ln2 = "en"
            this.innerHTML = "чото там"
            document.querySelector(".test.label.word").innerHTML = "укр"
            load_task()
        }
        else{
            ln1 = "en"
            ln2 = "ua"
            this.innerHTML = "чото там"
            document.querySelector(".test.label.word").innerHTML = "англ"
            load_task()
        }
    })
    document.querySelector(".test.button.primary.word").addEventListener("click", function(){
        ...
        right_ans.innerHTML = rezult + tasks[index][ln2]
        setTimeout(function(){
            right_ans.innerHTML = ""
            my_ans.value = ""
            index++
